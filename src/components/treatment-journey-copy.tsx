...
const TreatmentJourneyCopy = ({ data }: { data: ITreatmentJourney }) => {
  ...
  useGSAP(
    () => {
      ...
      items.forEach((item, index) => {
        ...
        tl2.add(() => {}, "+=2"); // Do nothing for 2 seconds
        tl2.to(item, {
    ...
    <div ref={mainRef} className="home py-5 lg:py-10">
      <Container>
        <div className="max-w-[48rem] mx-auto mb-8">
          ...
          {data.para && <p className="text-center text-[#333]">{data.para}</p>}
        </div>
        <div className="main relative">
          {data.boxes.map((item, index) => (
            <div key={index} className="sticky top-[30%]">
              <div
                className={`item flex will-change-auto flex-col  md:flex-row ${
                  item.reverse
...
export default TreatmentJourneyCopy;
